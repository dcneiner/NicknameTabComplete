 ...
 */
 ...
 h.substr(0,d.start-c.length);h=h.substr(d.start);space=i.matches.length>1||h.length&&h[0]==" "?"":" ";a.val(e+i.value+space+h);d=d.start-c.length+i.value.length+space.length;if(f.fn.nicknameTabComplete.has_newline_bug&&!f.browser.msie){a=f(this).val().substr(0,d).split("\n").length-1;d+=a}d=a=d;if(this.setSelectionRange){this.focus();this.setSelectionRange(a,d)}else if(this.createTextRange){c=this.createTextRange();c.collapse(true);c.moveEnd("character",d);c.moveStart("character",a);c.select()}}g.preventDefault();
