...
(function(f){function k(b){return("selectionStart"in b&&function(){var g=b.selectionEnd-b.selectionStart;return{start:b.selectionStart,end:b.selectionEnd,length:g,text:b.value.substr(b.selectionStart,g)}}||document.selection&&function(){b.focus();var g=document.selection.createRange();if(g===null)return{start:0,end:b.value.length,length:0};var e=b.createTextRange(),a=e.duplicate();e.moveToBookmark(g.getBookmark());a.setEndPoint("EndToStart",e);return{start:a.text.length,end:a.text.length+g.text.length,
 ...
 function(b){b=f.extend({},f.fn.nicknameTabComplete.defaults,b);this.bind("keydown.nickname",function(g){var e=b;if(g.which===9){var a=f(this),h=a.val(),d=k(this),c="",i="";if(!d.length&&d.start){if(f.fn.nicknameTabComplete.has_newline_bug){c=this.value.substr(0,d.start);d.start-=c.split(/\r/).length-1}c=h.substr(0,d.start);if(e.nick_match.test(c)){c=c.match(e.nick_match)[1];i=l(c,e.nicknames);e=f.Event("nickname-complete");f.extend(e,i);e.caret=d.start;a.trigger(e);if(i.value&&!e.isDefaultPrevented()){e=
 h.substr(0,d.start-c.length);h=h.substr(d.start);space=i.matches.length>1||h.length&&h[0]==" "?"":" ";a.val(e+i.value+space+h);d=d.start-c.length+i.value.length+space.length;if(f.fn.nicknameTabComplete.has_newline_bug){a=f(this).val().substr(0,d).split(/\n/m).length-1;d+=a}d=a=d;if(this.setSelectionRange){this.focus();this.setSelectionRange(a,d)}else if(this.createTextRange){c=this.createTextRange();c.collapse(true);c.moveEnd("character",d);c.moveStart("character",a);c.select()}}g.preventDefault();
