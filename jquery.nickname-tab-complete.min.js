 ...
 */
 ...
 function(b){b=i.extend({},i.fn.nicknameTabComplete.defaults,b);return this.bind("keydown",function(d){var g=b;if(d.which===9){var a=i(this),f=a.val(),c=k(this),e="",h="";if(!c.length&&c.start){e=f.substr(0,c.start);if(g.nick_match.test(e)){e=e.match(g.nick_match)[1];h=l(e,g.nicknames);if(h.value){g=f.slice(0,c.start-e.length);f=f.slice(c.start);space=f.length&&f[0]==" "?"":" ";a.val(g+h.value+space+f);a=c.start-e.length+h.value.length+space.length;if(this.setSelectionRange){this.focus();this.setSelectionRange(a,
 a)}else if(this.createTextRange){c=this.createTextRange();c.collapse(true);c.moveEnd("character",a);c.moveStart("character",a);c.select()}}d.preventDefault()}}}})};i.fn.nicknameTabComplete.defaults={nicknames:[],nick_match:/@([-_a-z]*)$/i}})(jQuery);